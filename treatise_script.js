...
document.addEventListener('DOMContentLoaded', () => { // <<< OPENING BRACE FOR DOMContentLoaded
    ...
    // --- START: Interactive Dock Text Code (Revised Logic) ---
    console.log("Initializing interactive dock text (Revised Logic)...");
    ...
    if (!overlayTitle || !overlayTagline || !dock) {
        ...
        let currentLink = null; // Keep track of the currently hovered link

        // Add ONE mouseover listener to the PARENT dock container
        dock.addEventListener('mouseover', (event) => {
            // Find the closest ancestor 'a' tag with data attributes
            const targetLink = event.target.closest('a[data-title]');

            // If we found a valid link and it's different from the last one hovered
            if (targetLink && targetLink !== currentLink) {
                currentLink = targetLink; // Update the currently hovered link
                // console.log("Hovering over:", targetLink.getAttribute('data-title')); // Debug log
                const title = targetLink.getAttribute('data-title');
                const tagline = targetLink.getAttribute('data-tagline');
            ...
            // If the mouse is over the dock but not directly over a valid link (e.g., the gap)
            // We don't reset here, wait for mouseleave from the whole dock
        });
        ...
        dock.addEventListener('mouseleave', () => {
            ...
            currentLink = null; // Reset the tracked link
        ...
        console.log("Interactive dock text feature initialized successfully (Revised Logic).");
    }
    // --- END: Interactive Dock Text Code ---


    // --- START: Expand/Collapse Functionality ---
    // (Keep your existing code for this here)
    ...
    function showGlossaryPopup(event) {
        ...
        let top = window.scrollY + linkRect.bottom + 10;
    ...
    const setupGlossary = function() {
        console.log("Setting up glossary listeners...");
        ...
        if (glossaryLinks.length > 0) {
             ...
             console.log(`Added listeners to ${glossaryLinks.length} glossary links.`);
        } else {
